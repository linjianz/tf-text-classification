...
dir0 = '20170719_1'  # change it every time when training
...
def load_data(file1, file2):
    ...
    # Split by words
    data = data1 + data2
    data = [clean_str(str(sent)) for sent in data]
    # Generate labels
...
def main(_):
    data_raw, label = load_data('data/rt-polaritydata/rt-polarity.pos', 'data/rt-polaritydata/rt-polarity.neg')
    # Build vocabulary
    sequence_length = max([len(x.split(" ")) for x in data_raw])
    vocab_processor = learn.preprocessing.VocabularyProcessor(sequence_length)
    data = np.array(list(vocab_processor.fit_transform(data_raw)))
    # 将text转化为index，并padding成相同长度，这里发现了一个小bug（同一个单词会有不同的index？）
    vocab_processor.save(os.path.join(dir_model, 'vocab'))

    # Generate train/val set
    shuffle_index = np.random.permutation(np.arange(len(label)))
    data_shuffled = data[shuffle_index]
    label_shuffled = label[shuffle_index]
    data_t, label_t = data_shuffled[: int(0.9 * len(label))], label_shuffled[: int(0.9 * len(label))]
    data_v, label_v = data_shuffled[int(0.9 * len(label)):], label_shuffled[int(0.9 * len(label)):]
    model_data_t = Data(data_t, label_t)
    model = Net(sequence_length=sequence_length,
